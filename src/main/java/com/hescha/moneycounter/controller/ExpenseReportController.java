...
import com.hescha.moneycounter.service.ExpenseReportService;
...
@RequestMapping(ExpenseReportController.CURRENT_ADDRESS)
public class ExpenseReportController {
    public static final String API_PATH = "expensereport";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new ExpenseReport());
        } else {
            model.addAttribute("entity", service.read(id));
        }

        model.addAttribute("expenseItem_list", expenseItemService.readAll());

        return THYMELEAF_TEMPLATE_EDIT_PAGE;
    }

    @PostMapping
    public String save(@ModelAttribute ExpenseReport entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                ExpenseReport createdEntity = service.create(entity);
