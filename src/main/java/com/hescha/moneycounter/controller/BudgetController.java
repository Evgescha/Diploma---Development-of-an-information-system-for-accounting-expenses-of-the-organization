...
@RequestMapping(BudgetController.CURRENT_ADDRESS)
public class BudgetController {
    public static final String API_PATH = "budget";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Budget());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Budget entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Budget createdEntity = service.create(entity);
